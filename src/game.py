...
class Game:
    def __init__(
        ...
        self.playing = True
        self.phases = ["place_units", "move_units", "attack_state"]
        self.phase_idx = 0
        self.phase = self.phases[self.phase_idx]
        self.phase_timer = pg.time.get_ticks()
        self.world = World()
        self.player = Player(
            self.world.states.get("Pennsylvania"), self.world, (0, 0, 255)
        )
    ...
    def update(self) -> None:
        ...
        if self.world.battle_res is not None:
            ...
            if self.world.battle_res["victory"]:
                if self.world.battle_res["attacking_state"] in self.player.controlled_states:
                    self.player.conquer(self.world.battle_res["defending_state"])
